...
class Loongarch64RelativePatcherTest : public RelativePatcherTest {
 public:
  Loongarch64RelativePatcherTest()
      : RelativePatcherTest(InstructionSet::kLoongarch64, "default") { }
...
TEST_F(Loongarch64RelativePatcherTest, StringReference) {
...
TEST_F(Loongarch64RelativePatcherTest, StringBssEntry) {
  for (size_t start_cnops : {0, 1, 2, 3, 4, 5, 6, 7}) {
    for (size_t mid_cnops : {0, 1, 2, 3, 4, 5, 6, 7}) {
...
TEST_F(Loongarch64RelativePatcherTest, MethodBssEntry) {
  for (size_t start_cnops : {0, 1, 2, 3, 4, 5, 6, 7}) {
    for (size_t mid_cnops : {0, 1, 2, 3, 4, 5, 6, 7}) {
      for (uint32_t bss_begin : { 0x12345678u, -0x12345678u, 0x10000000u, 0x12345000u }) {
