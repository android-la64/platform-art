...
#include "relative_patcher_loongarch64.h"
...
Loongarch64RelativePatcher::Loongarch64RelativePatcher(
    [[maybe_unused]] RelativePatcherThunkProvider* thunk_provider,
    [[maybe_unused]] RelativePatcherTargetProvider* target_provider,
    [[maybe_unused]] const Loongarch64InstructionSetFeatures* features)
    : RelativePatcher() {
}

uint32_t Loongarch64RelativePatcher::ReserveSpace(
    uint32_t offset,
...
uint32_t Loongarch64RelativePatcher::ReserveSpaceEnd(uint32_t offset) {
  // TODO(loongarch64): Reduce code size for AOT by using shared trampolines for slow path
  // runtime calls across the entire oat file. These need space reserved here.
  return offset;
}

uint32_t Loongarch64RelativePatcher::WriteThunks([[maybe_unused]] OutputStream* out, uint32_t offset) {
  // TODO(loongarch64): Reduce code size for AOT by using shared trampolines for slow path
  // runtime calls across the entire oat file. These need to be written here.
  return offset;
}

void Loongarch64RelativePatcher::PatchCall([[maybe_unused]] std::vector<uint8_t>* code,
...
void Loongarch64RelativePatcher::PatchPcRelativeReference(std::vector<uint8_t>* code,
...
void Loongarch64RelativePatcher::PatchEntrypointCall([[maybe_unused]] std::vector<uint8_t>* code,
...
void Loongarch64RelativePatcher::PatchBakerReadBarrierBranch(
  ...
  UNIMPLEMENTED(FATAL) << "Baker read barrier branches are not used on loongarch64.";
}

std::vector<debug::MethodDebugInfo> Loongarch64RelativePatcher::GenerateThunkDebugInfo(
...
uint32_t Loongarch64RelativePatcher::PatchAuipc(uint32_t auipc, int32_t offset) {
...
void Loongarch64RelativePatcher::SetInsn(std::vector<uint8_t>* code, uint32_t offset, uint32_t value) {
...
uint32_t Loongarch64RelativePatcher::GetInsn(ArrayRef<const uint8_t> code, uint32_t offset) {
  DCHECK_LE(offset + 4u, code.size());
  DCHECK_ALIGNED(offset, 2u);
...
uint32_t Loongarch64RelativePatcher::GetInsn(std::vector<uint8_t, Alloc>* code, uint32_t offset) {
