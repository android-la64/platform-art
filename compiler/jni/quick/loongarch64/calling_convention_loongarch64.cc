...
size_t Loongarch64JniCallingConvention::OutFrameSize() const {
  ...
  size_t out_args_size = RoundUp(size, kLoongarch64StackAlignment);
...
ArrayRef<const ManagedRegister> Loongarch64JniCallingConvention::ArgumentScratchRegisters() const {
  ...
  scratch_regs = scratch_regs.SubArray(1u);
...
bool Loongarch64JniCallingConvention::RequiresSmallResultTypeExtension() const {
  // LoongArch64 calling convention leaves upper bits undefined for float, int < 64bit
  return false;
}
