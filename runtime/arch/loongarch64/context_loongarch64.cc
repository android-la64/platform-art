...
void Loongarch64Context::Reset() {
  ...
  sp_ = Loongarch64Context::kBadGprBase + SP;
  pc_ = Loongarch64Context::kBadGprBase + kPC;
  arg0_ = 0;
}

void Loongarch64Context::FillCalleeSaves(uint8_t* frame, const QuickMethodFrameInfo& frame_info) {
...
void Loongarch64Context::SetGPR(uint32_t reg, uintptr_t value) {
...
void Loongarch64Context::SetFPR(uint32_t reg, uintptr_t value) {
...
void Loongarch64Context::DoLongJump() {
  ...
  for (size_t i = 0; i < kNumberOfFRegisters; ++i) {
    fprs[i] = fprs_[i] != nullptr ? *fprs_[i] : Loongarch64Context::kBadFprBase + i;
  }
